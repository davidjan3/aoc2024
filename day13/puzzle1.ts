...
function getTokens(machine: Machine) {
  ...
  // That early decision made puzzle 2 very easy :)
...
async function main() {
  ...
  const machines = machinesStr.map<Machine>((s) => {
    ...
    return {
      ...
      prize: [
        parseInt(rows[2].match(/X\=\d+/)![0].replace("X=", "")) + 10000000000000,
        parseInt(rows[2].match(/Y\=\d+/)![0].replace("Y=", "")) + 10000000000000,
      ],
    };
  });
